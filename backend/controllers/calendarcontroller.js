...
const registercalendar=(req,res)=>{
    const {calendar_date,day_type,description}=req.body;
    ...
    db.query(sql,[student_id,calendar_date,day_type,description],(err,result)=>{
        ...
        res.status(201).json({
            message: 'Calender registered successfully'
        });
    });
};
...
const updatecalendar=(req,res)=>{
    const {calendar_date,day_type,description,old_calender_date}=req.body;
    const student_id=req.student_id;
